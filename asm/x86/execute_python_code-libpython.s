...
execute_python_code_main:
	...
	# push r14
	# mov ebx, [BASEADDRESS:.+/libpython[0-9\.so]+$:BASEADDRESS] + [RELATIVEOFFSET:Py_Initialize:RELATIVEOFFSET]
	# pop r14
	...
	xor edi, edi
	xor esi, esi
	mov ebx, [BASEADDRESS:.+/libpython[0-9\.so]+$:BASEADDRESS] + [RELATIVEOFFSET:PyGILState_Ensure:RELATIVEOFFSET]
	call ebx
	mov ebx, arbitrary_read_write_data_address[rip]
	mov [ebx], eax
	//pop r14
	...
	add edi, 32
	xor ecx, ecx
	mov ebx, [BASEADDRESS:.+/libpython[0-9\.so]+$:BASEADDRESS] + [RELATIVEOFFSET:PyRun_SimpleStringFlags:RELATIVEOFFSET]
	call ebx
	pop ecx
	//pop r14
	...
	//mov edi, eax
	xor esi, esi
	mov ebx, [BASEADDRESS:.+/libpython[0-9\.so]+$:BASEADDRESS] + [RELATIVEOFFSET:PyGILState_Release:RELATIVEOFFSET]
	call ebx
	//pop r14
	...
	# mov eax, 0
	# mov edi, eax
	# mov ebx, [BASEADDRESS:.+/libpython[0-9\.so]+$:BASEADDRESS] + [RELATIVEOFFSET:Py_Finalize:RELATIVEOFFSET]
	# call ebx
	# pop r14
